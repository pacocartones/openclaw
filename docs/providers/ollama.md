...
| Code Mode preference | The exact `devstral-small-2:24b` tag is preferred under `tools.codeMode: "auto"` only when `/api/show` reports both `completion` and `tools`; it keeps file operations behind the compact `exec`/`wait` bridge. Aliases, other tags, and other Ollama models keep normal tool exposure.       |
